...
distancePermutationInterchange <- function(x, y){
	N=length(x) #==length(y)
	x<-y[order(x)]
	result <- .C("permutationDistanceInterchange", as.integer(x), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	(N-result$ret) / (N-1) 
}
...
distancePermutationLCSeq <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistanceLongestCommonSubsequence", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	(N-result$ret)/(N-1) # N-1 is for PERMUTATIONS only, because two permutations have always at least a common string/sequence of length one. 
...
distancePermutationLCStr <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistanceLongestCommonSubstring", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	(N-result$ret)/(N-1)
}
...
distancePermutationLevenshtein <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistanceLevenshtein", as.integer(x), as.integer(y), as.integer(N+1), ret=integer(1), PACKAGE="CEGO")
	result$ret/N
}
...
distancePermutationSwap <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistanceSwap", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	2*result$ret / (N^2 - N)
}
...
distancePermutationR <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistanceR", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	result$ret / (N - 1)
}
...
distancePermutationAdjacency <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistanceAdjacency", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	(N-result$ret-1) / (N - 1)
}
...
distancePermutationPosition <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistancePosition", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	dis <- result$ret
...
distancePermutationPosition2 <- function(x, y){
	N=length(x) #==length(y)
	result <- .C("permutationDistancePosition2", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	result$ret / ((N^3-N)/3) 
}
...
distancePermutationHamming <- function(x, y){
	#N=length(x) #==length(y)
	#result <- .C("permutationDistanceHamming", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO") #this works,but is actually slower than a pure R implementation, as below. at least for permutation length 30
	#result$ret / N
	sum(x != y)/length(x)
}
...
distancePermutationEuclidean <- function(x, y){
	N=length(x) #==length(y)
	#result <- .C("permutationDistanceEuclidean", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO") #this works,but is actually slower than a pure R implementation, as below. at least for permutation length 30
	#mdis <- sqrt(result$ret)
	mdis <- sqrt(sum((x-y)^2))
	if(N%%2){ #scale to [0;1] in case of odd N
		n=(N-1)/2
...
distancePermutationLee <- function(x,y){
	N <- length(x)
	result <- .C("permutationDistanceLee", as.integer(x), as.integer(y), as.integer(N), ret=integer(1), PACKAGE="CEGO") #this works,but is actually slower than a pure R implementation, as below. at least for permutation length 30
	mdis <- result$ret
...
#' 
#' The Insert Distance is an edit distance. It counts the minimum number of insert operations
#' required to transform one permutation into another. An insert operation shifts one element to a new position.
#' All other elements move accordingly to make place for the element. E.g., the following shows a single insert move that
#' sorts the corresponding permutation: 1 4 2 3 5 -> 1 2 3 4 5. This distance is also called Ulam's metric and is the same
#' as the distance measure based on the longest common subsequence (\code{\link{distancePermutationLCSeq}}). Note, that
#' the implementations differ. Insert distance scales better for longer permutations, whereas LCSeq works better for shorter
#' permutations.
#'
#' @param x first permutation (integer vector)
#' @param y second permutation (integer vector)
...
distancePermutationInsert <- function(x, y){
	N=length(x) #==length(y)
	x<-order(y)[x] #Important note: this line means "p1 * p2^{-1}". Schiavinotto say it should be  "p2^{-1} * p1" . That seems to be a typo/error. (of course it does not matter which permutation is is inverted. but the inverted permutation should ALWAYS be at the right.)
	result <- .C("longestIncreasingSubsequence", as.integer(x), as.integer(N), ret=integer(1), PACKAGE="CEGO")
	(N-result$ret)/(N-1)
}
...
#'
#' Calculate the longest increasing subsequence (LIS) in a vector (e.g., a permutation).
#'
...
#'
#' @examples
#' longestIncreasingSubsequence(1:5) 
#' longestIncreasingSubsequence(5:1)
#' longestIncreasingSubsequence(c(1,2,4,3,5))
#' longestIncreasingSubsequence(c(1,2,4,3,5,1,2,4,3,2,9)) 
#'
#' @export
#' @useDynLib CEGO
###################################################################################
longestIncreasingSubsequence <- function(x){
	N=length(x) #==length(y)
...
lexicographicPermutationOrderNumber <- function(x){
	N <- length(x)
	result <- .C("lexPermOrder", as.integer(x), as.integer(N), ret=integer(N), PACKAGE="CEGO")
	sum(result$ret*factorial((N-1):0))+1
}